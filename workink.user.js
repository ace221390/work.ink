...
(async function () {
    ...
    function _createRedirectNotice(dest) {
        try {
            ...
            // safe assignment as textContent (no HTML injection)
            const shortDest = (typeof dest === 'string' ? dest : '') || '';
            // show only host + path, keep it short
            try {
            ...
            // attach to document in a safe place
            ...
            requestAnimationFrame(() => {
                ...
                container.style.transform = container.style.transform.replace(' translateY(-6px)', ' translateY(0)');
            });

            // Simple countdown display synchronized with the 2s wait in finalRedirect
            ...
            // return a removal helper
            return {
                remove: () => {
                    try {
                        container.style.opacity = '0';
                        container.style.transform = container.style.transform.replace(' translateY(0)', ' translateY(-6px)');
            ...
            // silently ignore UI failures to avoid breaking redirect flow
            return { remove: ()=>{} };
        }
    }
    ...
    if (location.hostname === 'work.ink') {
        ...
        async function finalRedirect() {
            ...
            // show nice UI notice before redirecting
            const notice = _createRedirectNotice(dest);
            try {
                await new Promise(resolve => setTimeout(resolve, 2000));
            } finally {
                // remove notice just before navigating (if it's still there)
